...
/**
 * 
 * BNB Sent to this contract will be used to automatically buy USELESS
 * And deliver it to an input address. 8% of this transaction will be used to provide 
 * Liquidity Pairs to USELESS
 *
 */
contract UselessArbitrage is Context, Ownable {
  ...
  // fee taken out of USELSS Transfer
  uint8 public _uselessFee = 4;
  // fee taken out of BNB Transfer
  uint8 public _bnbFee = 4;
  ...
  event SentUselessToRecipient(
    uint256 uselessAmount
  );
  
  event SentPairToFurnace(
    uint256 uselessAmount,
    uint256 bnbAmount
  );
  
  function buyUselessSendToAddress(uint256 bnbAmount, address receiver) public onlyOwner{
    ...
    // how much BNB are we sending
    uint256 transferAmt = calculateTransferAmount(bnbAmount, _bnbFee);
    
    // how much USELESS did we have in this contract already
    uint256 initialUselessBalance = IERC20(_uselessAddr).balanceOf(address(this));
    
    // buy USELESS with 96% of the BNB and store in contract
    buyUselessStoreInContract(transferAmt);
    
    // how much USELESS did we just receive?
    uint256 uselessPurchased = IERC20(_uselessAddr).balanceOf(address(this)).sub(initialUselessBalance);
    
    // remove 4% of USELESS to send to Furnace, send the rest to user
    uint256 uselessToSend = calculateTransferAmount(uselessPurchased, _uselessFee);
    
    // send 96% of USELESS Received to the recipient
    IERC20(_uselessAddr).transfer(receiver, uselessToSend);
    
    emit SentUselessToRecipient(uselessToSend);

    // we now have 4% USELESS and 4% BNB, send the results to the Furnace if they hit threshholds
    if (address(this).balance >= bnbThreshold) {
        uint256 uselessBal = IERC20(_uselessAddr).balanceOf(address(this));
        sendToFurnace(uselessBal, bnbThreshold);
    }
  }
  
  function sendToFurnace(uint256 uselessAmount, uint256 bnbAmount) private {
      
      // send Useless to furnace
      IERC20(_uselessAddr).transfer(_uselessFurnace, uselessAmount);
      
       // send BNB to Furnace, allowing low level call
       (bool success, ) = _uselessFurnace.call{ value: bnbAmount }("");
        require(success, "Address: unable to send value, recipient may have reverted");
        
        emit SentPairToFurnace(uselessAmount, bnbAmount);
  }
   ...
   */ 
  function buyUselessStoreInContract(uint256 bnbAmount) private {
  ...
  /** Calculates the Transfer Rate for a given amount and tax*/
  function calculateTransferAmount(uint256 amount, uint8 fee) private pure returns (uint256){
     return amount.sub((amount.mul(fee)).div(10**2));
  }
  /**
